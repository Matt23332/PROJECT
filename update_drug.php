...
if (isset($_GET['searchDrug'])) {
  ...
  if (mysqli_num_rows($result) > 0) {
    ...
    echo '<input type="hidden" name="original_name" value="' .$row['drug_name'] . '">';
    echo '<input type="text" name="drug_name" value="' . $row['drug_name'] . '">';
...
if (isset($_POST['updateDrug'])) {
  ...
  $updateQuery = "UPDATE drug SET drug_name='$drug_name', formula='$formula', price='$price', company_name='$company_name', manufacture_date='$manufacture_date', expiry_date='$expiry_date', quantity='$quantity' WHERE drug_name='$original_name'";
